...
if __name__ == '__main__':
    ...
    # Get only patients known to pylidc (i.e., that have scans)
    valid_ids = set(scan.patient_id for scan in pl.query(pl.Scan).all())

    # Filter your local folder list to only those in pylidc's database
    LIDC_IDRI_list = [f for f in os.listdir(DICOM_DIR) if f in valid_ids]
    LIDC_IDRI_list.sort()
